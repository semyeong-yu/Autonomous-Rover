...
std::vector<Node> GraphPlanner::dijkstra(int start, int target)
{
    ...
    // if (!path.empty() && path.front().id != start)
    if (path.empty())
    {
...
{
    // std::vector<std::pair<int, double>> dist(nodes.size(), std::make_pair(0.0, std::numeric_limits<double>::max()));
    ...
    while (!pq.empty())
    {
        ...
        // if (nodes[currentNode - 1].visited) {       //! visited node는 search 하지 않는다. 오류 수정
        // continue;
        // }
    ...
    for(auto dist_buf : mine_dist_vector)
    {
        ...
        std::cout << "mine_node_id : " << mine_node_id << " mine_dist : " << mine_dist << std::endl;
    }
...
void GraphPlanner::Plan()
{
    ...
    // TODO : How to select target node -> minimum distance order

    if(bGlobalPlanTrigger)
        index = 0;
    if (!bUseRvizGoal && !bGetMineGoal || bGlobalPlanTrigger)
    {
        goal_node = findNearestMineNode(start_node);
        // ROS_WARN_THROTTLE(0.5, "[MINE SEARCH] START NODE %d, TARGET NODE %d", start_node, goal_node);
        ROS_WARN("[MINE SEARCH] START NODE %d, TARGET NODE %d", start_node, goal_node);
        bGetMineGoal = true;
    }
    else if (bUseRvizGoal && !bGetMineGoal)
    {
        ROS_WARN_THROTTLE(0.5, "[RVIZ GOAL] START NODE %d, TARGET NODE %d", start_node, goal_node);
    }


    if (bGetRvizGoal || (bGetMineGoal && bMineSearchTrigger) || (bGlobalPlanTrigger))
    {
        ROS_WARN_THROTTLE(0.5, "RUN DIJKSTRA");
        prev_solution.clear();
        
        std::vector<Node> shortestPath = dijkstra(start_node, goal_node);
        std::cout << "shortestPath size : " << shortestPath.size() << std::endl;
        prev_solution = shortestPath;
        bMineSearchTrigger = false;
        bGlobalPlanDone = true;
        GraphTrigger.data = true;
        index++;
    }   
    else if(index < 200)
    {
    ...
    new_graph_pub.publish(GraphTrigger);

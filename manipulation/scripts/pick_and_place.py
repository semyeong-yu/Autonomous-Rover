...
class PickandPlace():
    def __init__(self):
        ...
        rospy.Subscriber('/target_plan', String, self.plan_callback, queue_size=1) #
    ...
    def plan_callback(self, msg):  
        plan_message = msg.data
        self.mode = plan_message
        
    def arm_planning(self):
        self.target = self.inverse_kinematics.setPitchRanges((self.target_pose.pose.position.x, self.target_pose.pose.position.y, self.target_pose.pose.position.z), -145, -180, 0)
        # target = self.inverse_kinematics.setPitchRanges(self.init_pose)
        if self.target:
            servo_data = self.target[1]
    ...
    def init(self):
        self.target = self.inverse_kinematics.setPitchRanges(self.init_pose, -90, -180, 0)
        if self.target: 
            servo_data = self.target[1]
            bus_servo_control.set_servos(self.joints_pub, 1500, ((1, 50), (2, 500), (3, servo_data['servo3']),
                            (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
    ...
    def grasping(self):
        #target = self.inverse_kinematics.setPitchRanges((target_x, target_y, target_z), -145, -180, 0)
            ...
            #servo_data = target[1]
        servo_data = self.target[1]
        bus_servo_control.set_servos(self.joints_pub, 1500, ((1, 500), (2, 500), (3, servo_data['servo3']),
                            (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
        time.sleep(1.5)
        
    def basket_planning(self):
        bus_servo_control.set_servos(self.joints_pub, 1500, ((1, 500), (2, 500), (3, 900),
                            (4, 365),(5, 460),(6, 500)))
        time.sleep(1.5)
